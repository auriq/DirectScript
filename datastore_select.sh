...
echo "command4: view NASA NEX data as if it is on a local drive"
ess ls

###select azure blob datastore#######

echo "command5: add an azure blob bucket to datastore"
ess select blob://indexdev --credentials cborys_azure.csv

echo "command6: list content of the blob  root dir"
ess ls

echo "command7: switch back to s3 datastore"
